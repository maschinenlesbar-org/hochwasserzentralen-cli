  ...
  new Promise<HttpResponse>((resolve, reject) => {
      ...
      (res) => {
        ...
        res.on("data", (chunk: Buffer) => {
          ...
          if (maxBytes !== undefined && received > maxBytes) {
            aborted = true;
            res.destroy();
            reject(new HochwasserzentralenNetworkError(`Response exceeded maxResponseBytes (${maxBytes})`));
            return;
          }
          chunks.push(chunk);
        });
        ...
        res.on("error", (err) => {
          if (aborted) return;
          reject(new HochwasserzentralenNetworkError(`Response stream error: ${err.message}`, { cause: err }));
        });
      },
    );
    ...
    if (request.timeoutMs && request.timeoutMs > 0) {
      req.setTimeout(request.timeoutMs, () => {
        req.destroy(new HochwasserzentralenNetworkError(`Request timed out after ${request.timeoutMs}ms`));
      });
    }

    req.on("error", (err) => {
      reject(
        err instanceof HochwasserzentralenNetworkError
          ? err
          : new HochwasserzentralenNetworkError(err.message, { cause: err }),
      );
    });
