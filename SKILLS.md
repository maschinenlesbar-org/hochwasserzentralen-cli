...
| **hochwasser-station-situation** | Condenses the ~1200 LHP gauges into a per-state or per-river flood-classification verdict via the built-in `situation` aggregate. | "flood situation on the Rhine?", "wie ist die Hochwasserlage?", "which rivers are flooding?" |
...
```
/plugin marketplace add maschinenlesbar-org/hochwasserzentralen-cli
/plugin install hochwasser@hochwasser-skills
```

The first command registers the marketplace; the second installs the `hochwasser` plugin,
